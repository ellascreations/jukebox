...
export default defineEventHandler(async (event) => {
  ...
  if (!query.code || !query.state || query.state !== savedState || !hostId) {
    throw createError({ statusCode: 400, statusMessage: 'Invalid Spotify OAuth state' })
  }
  ...
  const profile = await $fetch<any>('https://api.spotify.com/v1/me', {
